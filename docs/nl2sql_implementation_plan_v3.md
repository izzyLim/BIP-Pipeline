# NL2SQL 사내 적용 구현 계획 v3 — LangGraph + QuerySpec

> **목적:** 사내 Oracle 19c 환경에서 NL2SQL을 실제로 구축하는 **단계별 실행 가이드**. BIP-Pipeline 검증 경험 + WrenAI/Cube 후보 검토 + LangGraph 직접 구현 결정 이력을 단일 문서로 통합.
>
> **v2와의 관계:** `nl2sql_implementation_plan_v2.md`는 OM + Cube + LangGraph 조합으로 진행하던 계획이었으나, Cube 검증 결과 팩트-팩트 JOIN 한계가 발견되어 본 v3 (LangGraph 직접 구현)로 방향 전환. v2는 의사결정 이력 보존 차원에서 유지.
>
> **이 문서가 답하는 것:**
> - 어떤 아키텍처를 왜 골랐는가 (1장)
...
> - `docs/nl2sql_implementation_plan_v2.md` — v2 (OM + Cube + Agent) 의사결정 이력
