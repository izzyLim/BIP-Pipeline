...
### 5-4. View 설계 방법론 — 질문 유형 기반 (체계적 접근)

#### 두 가지 접근법

| | Reactive (테스트 기반) | Proactive (질문 유형 기반) |
|---|---|---|
| **순서** | 테스트 → 실패 발견 → View 생성 | 질문 분류 → 갭 분석 → View 설계 → 테스트 |
| **장점** | 쓸모없는 View 안 만듦 | 커버리지 보장, 누락 방지 |
| **단점** | 테스트 안 한 유형은 빠짐 | 과설계 위험 |
| **적합** | 프로토타입/초기 | 사내 적용/프로덕션 |

**프로토타입은 Reactive로 시작하되, 프로덕션 적용 전에 Proactive로 전환.**

#### Proactive 방법: 질문 유형 → 데이터 Shape → 갭 분석

```
Step 1: 사용자가 물어볼 질문 유형을 나열
Step 2: 각 유형에 필요한 데이터 shape(컬럼, Grain, 계산) 정의
Step 3: 기존 Gold/Raw로 커버 가능한지 판단
Step 4: 커버 안 되면 View로 해결할지, 에이전트로 해결할지 결정
Step 5: View 생성 + 테스트
```

#### BIP 실제 적용 (질문 유형 → View 매핑표)

| 질문 유형 | 예시 | 필요 shape | Gold 커버? | 해결 |
|---------|------|----------|:---:|------|
| 종목 단순 조회 | "삼성전자 종가" | 시세 1행 | ✅ | - |
| 밸류에이션 조회 | "PER 낮은 종목" | 밸류에이션 | ✅ | - |
| **분류 검색** | "저평가주" | **boolean flag** | ❌ | `v_valuation_signals` |
| **기술 신호** | "과매도 종목" | **boolean flag** | ❌ | `v_technical_signals` |
| **파생 계산** | "거래대금", "이격도" | **pre-compute** | ❌ | `v_technical_signals` |
| **수급 금액** | "외국인 순매수 금액" | **금액 환산** | ❌ | `v_flow_signals` |
| **크로스 Grain** | "PER 낮은 종목 주가" | **grain 일치** | ❌ | `v_latest_valuation` |
| 매크로 조회 | "VIX 지수" | 매크로 피벗 | ✅ | - |
| 매크로 분류 | "VIX 공포 구간" | boolean flag | ❌ | ⚠️ 미구현 |
| 섹터 집계 | "업종별 평균 PER" | 섹터 GROUP BY | ❌ | ⚠️ 미구현 |
| 실적+추정 | "2023-2026 순이익률" | data_type 통합 | ✅ (통합) | - |
| 종목 비교 | "삼성 vs 하이닉스" | 멀티 종목 | ✅ | - |
| 시계열 이벤트 | "골든크로스 후 수익률" | 멀티스텝 | ❌ | Phase 3 에이전트 |
| 인과/추론 | "금리 인상 영향" | 멀티 소스 | ❌ | Phase 3 에이전트 |

#### BIP에서 만든 4개 View의 판단 근거

| View | 만든 이유 | 판단 기준 |
|------|---------|---------|
| `v_latest_valuation` | 크로스 Gold 질문(PER+주가) 테스트 실패 | **Grain 불일치** — daily ↔ annual 직접 JOIN 불가 |
| `v_valuation_signals__v1` | "저평가주" 질문에 LLM이 매번 PER<10 추론 불안정 | **LLM 반복 추론 → boolean 고정** |
| `v_technical_signals__v1` | "과매도", "이격도" 계산이 description에만 존재 | **파생 계산 + boolean → 컬럼으로 고정** |
| `v_flow_signals__v1` | "외국인 순매수 금액" 계산이 매번 필요 | **파생 계산(금액 환산) → 컬럼으로 고정** |

#### View 생성 판단 기준 요약

```
View를 만들어야 하는 3가지 신호:

1. "LLM이 같은 계산을 매번 다르게 추론한다"
   → 계산식을 View 컬럼으로 고정 (boolean flag 또는 파생)

2. "두 테이블의 Grain이 달라서 JOIN이 안 된다"
   → Grain 변환 View (v_latest_*)

3. "자주 묻는 질문인데 Gold에 해당 컬럼이 없다"
   → 파생 계산을 View 컬럼으로 추가

View를 만들면 안 되는 2가지 신호:

1. "한 번만 물어볼 특이한 질문이다"
   → SQL Pair로 해결 또는 무시

2. "멀티스텝 추론이 필요하다" (UNION, 순위 후 필터, 이벤트 탐지)
   → Phase 3 에이전트에서 해결
```
