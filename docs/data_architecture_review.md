...
Layer 1: Raw / Operational
  stock_info              — 종목 메타데이터
  stock_price_1d/1m/1wk/1mo  — OHLCV 시세
...
Layer 3: Gold / Serving     ← 신규 추가 권장
  gold_stock_daily_snapshot — 멀티 조인 pre-joined 와이드 테이블
  gold_macro_daily_snapshot — 매크로 지표 통합 뷰

Layer 4: Semantic           ← NL2SQL 준비 위해 필요
  비즈니스 용어집, Join 경로, 단위 정의, 지표 계산식
```
...
| 테이블 | 상태 | 설명 | 주요 제약 |
|--------|------|------|-----------|
| `stock_price_1m` | ✅ | 1분봉 | UNIQUE(ticker, timestamp) |
| `stock_price_1d` | ✅ | 일봉 | UNIQUE(ticker, timestamp) |
| `stock_price_1wk` | ✅ | 주봉 | UNIQUE(ticker, timestamp) |
| `stock_price_1mo` | ✅ | 월봉 | UNIQUE(ticker, timestamp) |
...
| 02 | `load_historical_*` | Yahoo Finance | `stock_price_1d/1m` | 수동 |
...
| `stock_price_1wk` | `ticker` | → | `stock_info` | `ticker` |
| `stock_price_1mo` | `ticker` | → | `stock_info` | `ticker` |
...
```
stock_info (ticker) ──┬── stock_price_1d (ticker)
...
| 2-5 | OM 메타데이터 정비 (테이블/컬럼 설명, 태그, 용어집) | 아키텍처 | 🟡 진행 예정 |
