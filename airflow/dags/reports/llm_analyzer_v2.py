...
def call_haiku(prompt: str) -> str:
    """
    Haiku 모델로 빠른 요약/인사이트 생성 (비용 효율적)
...
어제 무슨 일이 있었는지는 이미 별도로 요약됩니다. 여기서는 그 결과로 **오늘/이번 주 어떤 흐름이 예상되는지**에 집중해주세요.
...
- `### 📈 [전망] 오늘 한국 시장 예상 (09:00 개장) [🟡]`
- `### 🔬 반도체 섹터 심층 분석 [🟢]`

---

### 📌 어제의 핵심 (3-5줄)
어제 시장에서 실제로 일어난 일을 사실 중심으로 요약합니다.
- 주요 지수 등락과 원인 (수치 포함)
- 가장 큰 이슈가 된 뉴스나 이벤트
- 섹터/종목 중 특이 움직임
...
⚠️ **이 섹션에서는 미국 시장을 언급하지 마세요!** 한국장이 먼저 마감되었으므로, 어제 미국장 움직임은 어제 한국장에 영향을 줄 수 없습니다.
...
이 데이터는 한국장 마감 **이후**에 발생한 것입니다. 한국 시간 오늘 새벽 6시에 마감되었습니다.
...
✅ **이 섹션에서만** 미국 시장이 한국에 미칠 영향을 분석하세요!
...
def format_night_futures(data: Dict) -> str:
    ...
    if ewy:
        ...
        lines.append(f"**한국 ETF (EWY)**: ${ewy['value']:.2f} ({change_pct:+.2f}%, {direction}) [{price_source}]")
