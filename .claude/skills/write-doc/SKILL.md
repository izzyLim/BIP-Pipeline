...
- Mermaid 다이어그램: 최소 7개 (아키텍처, 동작 원리, 컴포넌트 관계, 시퀀스, 비교 등)
  - **필수 포함: "전체 흐름 다이어그램"** — 데이터/요청/종목이 단계별로 어떻게 변화하는지(funnel, 파이프라인, 라이프사이클 등). 각 단계마다 수치/상태 변화를 표시
  - 예: "2,800종목 → 시총필터 724개 → 프리셋 90개 → 품질필터 35개 → 최종 10개"
  - 예: "API 요청 → 인증 → 라우팅 → 비즈니스 로직 → DB → 응답"
  - 예: "Raw 데이터 → 변환 → 검증 → 저장 → 인덱싱 → 검색 가능"

  **다이어그램 가독성 규칙 (가로 길어짐 방지):**
  - **노드 5개 이상이면 `flowchart TD`(세로) 사용**, `LR`(가로)는 4개 이하일 때만
  - **노드 라벨 길어지면 `<br/>`로 줄바꿈** (예: `"단계명<br/>세부설명"`)
  - **5단계 초과 funnel은 그룹핑** — `subgraph`로 묶어서 단계별 영역 구분
    ```
    flowchart TD
        subgraph Phase1["Phase 1 (724 → 35)"]
            A --> B --> C
        end
        subgraph Phase2["Phase 2 (35 → 10)"]
            D --> E
        end
        Phase1 --> Phase2
    ```
  - **라벨에 수치 포함** — "약 90개" 같은 수치를 노드 안에 직접 적기
  - **한 다이어그램 노드 수 8~12개로 제한** — 더 많으면 2개로 분리 (개요용 + 상세용)
  - **시퀀스 다이어그램은 actor 5명 이하** — 더 많으면 그룹화
...
- [ ] Mermaid 다이어그램 7개 이상
- [ ] **전체 흐름 다이어그램 필수 포함** (단계별 데이터/요청/종목 수 변화 funnel 또는 라이프사이클)
